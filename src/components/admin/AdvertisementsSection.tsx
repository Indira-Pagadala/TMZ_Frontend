...
export function AdvertisementsSection(): JSX.Element {
  ...
  const openAdd = () => { setEditing(null); setForm(tab === 'ads' ? { title: '', image_url: '', target_url: '', ad_slot_id: '', status: 'ACTIVE', starts_at: '', ends_at: '' } : { name: '', slug: '', placement: '', description: '', is_active: true }); setModalOpen(true); };
  ...
  const save = async () => {
    try {
      if (tab === 'ads') {
        const data = { title: String(form.title), image_url: String(form.image_url), target_url: String(form.target_url), ad_slot_id: form.ad_slot_id ? String(form.ad_slot_id) : null, status: String(form.status) as AdStatus, starts_at: form.starts_at ? String(form.starts_at) : null, ends_at: form.ends_at ? String(form.ends_at) : null };
