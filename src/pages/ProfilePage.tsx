...
import {
  ...
  fetchQuizStats, fetchUserOpinions, fetchAllBadges, fetchAchievementHistory,
  fetchArticlesCompletedCount, fetchOpinionsCount, fetchShareCardsCount,
} from '@/lib/api';
import type {
...
function ProfileOverview() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchLevels(),
      fetchLevelByNumber(profile?.level ?? 1),
      fetchArticlesCompletedCount(uid),
      fetchQuizStats(uid),
      fetchOpinionsCount(uid),
      fetchShareCardsCount(uid),
      fetchReadingHistory(uid),
      fetchCompletedArticles(uid),
      fetchSavedArticles(uid),
      fetchUserOpinions(uid),
      fetchAchievementHistory(uid),
      fetchAllBadges(uid),
      fetchUserBadges(uid),
      fetchCompletionCards(uid),
    ]).then((results) => {
      const [lvls, curLvl, completed, qStats, opCount, scCount, history, done, saved, opinions, achieve, badges, userBadges, cards] = results;
      setCurrentLevel(curLvl);
      setNextLevel(lvls.find((l) => l.level_number === (profile?.level ?? 1) + 1) ?? null);
      setArticlesCompleted(completed);
      setQuizStats(qStats);
      setOpinionsCount(opCount);
      setShareCardsCount(scCount);
      setReadingHistory(history);
      setCompletedArticles(done);
      setSavedArticles(saved);
      setUserOpinions(opinions);
      setAchievements(achieve);
      setAllBadges(badges.all);
      setEarnedBadgeIds(badges.earned);
      setBadgesEarned(userBadges.length);
      setCompletionCards(cards);
    }).catch(() => {
      showToast('Could not load profile data', 'error');
    });
  }, [user, profile?.level, showToast]);
